...
class BuckshotGUI:
            ...
            try:
                with open(self.data_file, 'r') as f:
                    ...
                    self.learn_data = json.loads(content) if content else {
                        'patronen': {'rot': 1.0, 'blau': 1.0},
                        'items': {'richtig': {}, 'falsch': {}}
                    }
    ...
    def save_learning_data(self):
        ...
        print("[DEBUG] Lernverhalten gespeichert:", self.learn_data)
    ...
    def get_weighted_prediction(self):
        ...
        weighted_options = {
            color: self.counts[color] * self.learn_data['patronen'].get(color, 1.0)
    ...
    def update_round(self):
        ...
        self.invert_button = tk.Button(self.round_frame, text="INVERTER BENUTZT?", command=self.invert_prediction, bg="orange", fg="black", width=20, highlightthickness=2)
    ...
    def invert_prediction(self):
        if self.prediction:
            ...
            messagebox.showinfo("Vorhersage invertiert", f"Statt: Nächste Patrone: {alte.upper()} → Zu: {self.prediction.upper()} (nur diese Runde)")
            if hasattr(self, 'prediction_label'):
                self.prediction_label.config(text=f"Nächste Patrone: {self.prediction.upper()} (INVERTER AKTIV)")
    ...
    def evaluate_choice(self, gezogen, prediction):
        ...
        if prediction != gezogen:
            self.learn_data['patronen'][gezogen] += 1.0
        else:
            self.learn_data['patronen'][gezogen] += 0.1
